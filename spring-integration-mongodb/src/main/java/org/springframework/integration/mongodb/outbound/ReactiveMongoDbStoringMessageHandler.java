 ...
 * which writes Message payload into a MongoDb collection, using reactive MongoDb support,
 * The collection is identified by evaluation of the {@link #collectionNameExpression}.
 *
 * @author David Turanski
 *
 * @since 5.2.2
 */
public class ReactiveMongoDbStoringMessageHandler extends AbstractReplyProducingMessageHandler {
	 ...
	 * provided {@link ReactiveMongoOperations}
	 *
	...
	@Override
	protected Mono<?> handleRequestMessage(Message<?> requestMessage) {
		Assert.isTrue(this.initialized, "This class is not yet initialized. Invoke its afterPropertiesSet() method");
		String collectionName = this.collectionNameExpression.getValue(this.evaluationContext, requestMessage, String.class);
		Assert.notNull(collectionName, "'collectionNameExpression' must not evaluate to null");

		Object payload = requestMessage.getPayload();

		return this.mongoTemplate.save(payload, collectionName);
	}
}
